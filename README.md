...
## IMPORTANT ##########################################################
## The output of the program matcomp_HTML_ST is provided for informational purposes only and must not be used for any serious, commercial, or safety-critical undertaking. 
## The input data has not been validated or verified, and the user assumes all risks associated with its use.
## IMPORTANT ##########################################################

This is a single-page web application designed in the LCARS (Star Trek) style, intended for uploading, selecting, and comparing material data from a CSV file. 
The CSV file consists of materials data from MMPDS and is found under the folder "assets".

1. External Resources and Core Setup

Title: MMPDS ANALYZER

    CSV Parser: Includes the PapaParse JavaScript library (papaparse@5.4.1/papaparse.min.js) for
    reading CSV files directly in the browser.

    Styling: All styling is embedded within a <style> block.

    It uses numerous CSS variables (e.g., --cool, --tangerine, --evening) to define the LCARS color palette and 
    dimensions for bars, panels, and text.
...
The page uses a flexible layout consisting of a fixed-width Left Frame (Sidebar) and an expanding 
...
Left Sidebar (.left-frame):
    Contains the ship name button (USS Abot NCC-2C01) and several stacked, stylized buttons/panels for status and navigation.
...
Step 1: Initialize Dataset
    ...
    The script uses PapaParse to read the data, requiring the CSV to contain columns named 'Basis' and 'Ref' 
    to correctly identify the numerical data properties.
...
Step 3: Analysis Output (Results Area)
    ...
    Renders a table (comparison-table) showing the Property, Value A, Value B, and the color-coded Delta (Green for positive change, 
...
Why 2020? 
That was the start of COVID, and the beginning of the era of AI, and the true demise of BATS.
